...
# dataset. The `column` is optional: omit it to count all datapoints, or set
# it to scope to a specific metric.
...
# HISTOGRAM_COUNT counts events recorded in a histogram-typed metric column.
# The `column` is required and must reference a histogram metric.
...
# Saved queries + annotations so the queries can be referenced by boards and
# linked to from triggers.
